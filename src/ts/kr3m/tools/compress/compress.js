...
(function (kr3m) {
    ...
    (function (async) {
        var Switch = (function () {
            ...
            Switch.byThen = function (value, optionCallbacks, defaultCallback, callback) {
                var switchDone = function () { return callback && callback(); };
            ...
            Switch.by = function (value, optionCallbacks, defaultCallback) {
                if (optionCallbacks[value])
...
(function (kr3m) {
    ...
    (function (images) {
        var ImageMagick = (function () {
            ...
            ImageMagick.prototype.usePalette = function () {
            ...
            ImageMagick.prototype.flush = function (outputPath, callback) {
                ...
                process.exec(function (status) {
                    _this.args.pop();
                    if (status != kr3m.SUCCESS) {
                        logError("ImageMagic failed:");
                        logError.apply(void 0, [ImageMagick.convertPath].concat(_this.args));
    ...
    (function (tools) {
        ...
        (function (compress) {
            var Compress = (function () {
                ...
                Compress.prototype.compressPng = function (path, callback) {
                    log("  compressing png");
                    var magic = new kr3m.images.ImageMagick();
                    magic.image(path).usePalette().flush(path, function () { return callback(); });
                };
                Compress.prototype.runWithParameters = function (params) {
                    var _this = this;
                    ...
                    kr3m.util.File.crawlAsync(workPath, function (relativePath, next, isFolder, absolutePath) {
                        log("checking", relativePath);
                        var ext = kr3m.util.File.getExtension(relativePath).slice(1);
                        kr3m.async.Switch.byThen(ext, {
                            png: function (switchDone) { return _this.compressPng(workPath + "/" + relativePath, next); }
                        }, function (switchDone) {
                            log("  skipping");
                            switchDone();
                        }, next);
                    }, { wantFiles: true, wantFolders: true, recursive: true }, function () {
                        log("done");
                    });
                };
        ...
        })(compress = tools.compress || (tools.compress = {}));
