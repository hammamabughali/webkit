...
{
	export class CompetitionsTable
	{
		...
		*/
		...
		public getIterative(
			where:string,
			dataCallback:(vos:cuboro.tables.CompetitionVO[], callback:Callback) => void,
		...
		*/
		...
		{
			db.fetchPage("competitions", where, orderBy, joins, offset, limit, (rows:any[], totalCount:number) =>
			{
				for (var i = 0; i < rows.length; ++i)
				{
					rows[i].__proto__ = kr3m.util.Factory.getInstance().map(cuboro.tables.CompetitionVO).prototype;
		...
		*/
		public getDojo(
			params:any,
			callback:(forDojo:kr3m.dojo.GridQueryResponse<cuboro.tables.CompetitionVO>) => void,
			conditions:string[] = [],
			escapeArgs:any[] = [],
			errorCallback?:ErrorCallback):void
		{
			...
			{
				this.get(where, offset, limit, ordering, (vos:any) => callback(new kr3m.dojo.GridQueryResponse<cuboro.tables.CompetitionVO>(vos, "id", count, "id", sort)), errorCallback);
			}, errorCallback);
		}
		...
		public upsertBatch(
			vos:cuboro.tables.CompetitionVO[],
			callback?:Callback,
			errorCallback?:ErrorCallback):void
		{
			...
			db.upsertBatch("competitions", vos, () =>
			{
				for (var i = 0; i < vos.length; ++i)
					vos[i].postStore();
			...
			errorCallback?:ErrorCallback):void
		{
			...
			db.insertBatch("competitions", vos, () =>
			{
				for (var i = 0; i < vos.length; ++i)
					vos[i].postStore();
		...
		*/
		public getByIds(
			ids:number[],
			callback:(vosById:{[id:number]:cuboro.tables.CompetitionVO}) => void,
			errorCallback?:ErrorCallback):void
		{
