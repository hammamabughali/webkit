...
{
	export class RegionDomainsTable
	{
		...
		*/
		...
		public getIterative(
			where:string,
			dataCallback:(vos:cuboro.tables.RegionDomainVO[], callback:Callback) => void,
		...
		*/
		...
		{
			db.fetchPage("region_domains", where, orderBy, joins, offset, limit, (rows:any[], totalCount:number) =>
			{
				for (var i = 0; i < rows.length; ++i)
				{
					rows[i].__proto__ = kr3m.util.Factory.getInstance().map(cuboro.tables.RegionDomainVO).prototype;
		...
		*/
		public getDojo(
			params:any,
			callback:(forDojo:kr3m.dojo.GridQueryResponse<cuboro.tables.RegionDomainVO>) => void,
			conditions:string[] = [],
			escapeArgs:any[] = [],
			errorCallback?:ErrorCallback):void
		{
			...
			{
				this.get(where, offset, limit, ordering, (vos:any) => callback(new kr3m.dojo.GridQueryResponse<cuboro.tables.RegionDomainVO>(vos, "name", count, "name", sort)), errorCallback);
			}, errorCallback);
		}
		...
		public upsertBatch(
			vos:cuboro.tables.RegionDomainVO[],
			callback?:Callback,
			errorCallback?:ErrorCallback):void
		{
			errorCallback = this.wrapErrorCallback(errorCallback, "upsertBatch");
			vos = vos.slice();
			var noNameVos:cuboro.tables.RegionDomainVO[] = [];
			...
			db.upsertBatch("region_domains", vos, () =>
			{
				for (var i = 0; i < vos.length; ++i)
					vos[i].postStore();

				kr3m.async.Loop.forEach(noNameVos, (noNameVo, next) =>
				{
			...
			errorCallback?:ErrorCallback):void
		{
			...
			db.updateBatch("region_domains", vos, () =>
			{
				for (var i = 0; i < vos.length; ++i)
					vos[i].postStore();
			...
			errorCallback?:ErrorCallback):void
		{
			...
			db.insertBatch("region_domains", vos, () =>
			{
				for (var i = 0; i < vos.length; ++i)
					vos[i].postStore();
		...
		*/
		public getByNames(
			names:string[],
			callback:(vosByName:{[name:string]:cuboro.tables.RegionDomainVO}) => void,
			errorCallback?:ErrorCallback):void
		{
