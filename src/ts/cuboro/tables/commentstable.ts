...
module cuboro.tables
{
	export class CommentsTable
	{
		...
		*/
		...
		public getIterative(
			where:string,
			dataCallback:(vos:cuboro.tables.CommentVO[], callback:Callback) => void,
		...
		*/
		...
		{
			db.fetchPage("comments", where, orderBy, joins, offset, limit, (rows:any[], totalCount:number) =>
			{
				for (var i = 0; i < rows.length; ++i)
				{
					rows[i].__proto__ = kr3m.util.Factory.getInstance().map(cuboro.tables.CommentVO).prototype;
		...
		*/
		public getDojo(
			params:any,
			callback:(forDojo:kr3m.dojo.GridQueryResponse<cuboro.tables.CommentVO>) => void,
			conditions:string[] = [],
			escapeArgs:any[] = [],
			errorCallback?:ErrorCallback):void
		{
			...
			{
				this.get(where, offset, limit, ordering, (vos:any) => callback(new kr3m.dojo.GridQueryResponse<cuboro.tables.CommentVO>(vos, "id", count, "id", sort)), errorCallback);
			}, errorCallback);
		}
		...
		*/
		public getByIds(
			ids:number[],
			callback:(vosById:{[id:number]:cuboro.tables.CommentVO}) => void,
			errorCallback?:ErrorCallback):void
		{
