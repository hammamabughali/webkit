...
/// <reference path="../../cuboro/tables/sessionvo.ts"/>
	...
	export class SessionsTable
	{
		...
		*/
		...
		public getCount():void
		{
			...
			var sql = "SELECT COUNT(*) FROM `sessions` WHERE " + where;
		...
		public get(
			...
			limit:number,
			ordering:string[],
			callback:CB<cuboro.tables.SessionVO[]>,
			errorCallback:ErrorCallback):void;
		...
		public get(
			...
			limit:number,
			ordering:string[],
			callback:CB<cuboro.tables.SessionVO[]>):void;
		...
		public get():void
		{
			...
			var callback = <CB<cuboro.tables.SessionVO[]>> u.getFirstOfType(arguments, "function", 0, 0);
			var errorCallback = <ErrorCallback> u.getFirstOfType(arguments, "function", 0, 1);
			errorCallback = this.wrapErrorCallback(errorCallback, "get");

			var sql = "SELECT * FROM `sessions` WHERE " + whereSql;
			...
			db.fetchAll(sql, (rows:any[]) =>
			{
				...
				{
					rows[i].__proto__ = kr3m.util.Factory.getInstance().map(cuboro.tables.SessionVO).prototype;
					rows[i].postLoad();
				}
				callback(rows);
			}, errorCallback);
		...
		public getIterative(
			where:string,
			dataCallback:(vos:cuboro.tables.SessionVO[], callback:Callback) => void,
			doneCallback?:Callback,
			errorCallback?:ErrorCallback):void
		{
			errorCallback = this.wrapErrorCallback(errorCallback, "getIterative");
			where = where.replace(/^\s*where\s*/i, " ");
			var sql = "SELECT * FROM `sessions` WHERE " + where;
		...
		*/
			...
			errorCallback?:ErrorCallback):void
		{
			...
			db.updateBatch("sessions", rows, callback, db.defaultBatchSize, "id", errorCallback);
		}
		...
		*/
		...
		{
			db.fetchPage("sessions", where, orderBy, joins, offset, limit, (rows:any[], totalCount:number) =>
			{
				for (var i = 0; i < rows.length; ++i)
				{
					rows[i].__proto__ = kr3m.util.Factory.getInstance().map(cuboro.tables.SessionVO).prototype;
		...
		public fetchCol():void
		{
			...
			{
				var error = "invalid column name for table sessions: " + colName;
				if (errorCallback)
			...
			var sql = "SELECT " + distinctSql + "`" + colName + "` FROM `sessions` WHERE " + whereSql + orderSql + limitSql;
			...
			errorCallback?:ErrorCallback):void
		{
			...
			whereSql = whereSql.replace(/^\s*where\s*/i, " ");
			var sql = "SELECT `" + colName + "` FROM `sessions` WHERE " + whereSql + " LIMIT 1;";
			db.fetchOne(sql, callback, errorCallback);
		}
			...
			errorCallback?:ErrorCallback):void
		{
			...
			{
				var error = "invalid column name for table sessions: " + keyName;
				if (errorCallback)
					return errorCallback(error);
			...
			{
				var error = "invalid column name for table sessions: " + valueName;
				if (errorCallback)
					return errorCallback(error);
			...
			whereSql = whereSql.replace(/^\s*where\s*/i, " ");
			var sql = "SELECT `" + keyName + "`, `" + valueName + "` FROM `sessions` WHERE " + whereSql;
		...
		*/
		public getDojo(
			params:any,
			callback:(forDojo:kr3m.dojo.GridQueryResponse<cuboro.tables.SessionVO>) => void,
			conditions:string[] = [],
			escapeArgs:any[] = [],
			errorCallback?:ErrorCallback):void
		{
			...
			{
				this.get(where, offset, limit, ordering, (vos:any) => callback(new kr3m.dojo.GridQueryResponse<cuboro.tables.SessionVO>(vos, "id", count, "id", sort)), errorCallback);
			}, errorCallback);
		}
		...
		public upsertBatch(
			vos:cuboro.tables.SessionVO[],
			callback?:Callback,
			errorCallback?:ErrorCallback):void
		{
			errorCallback = this.wrapErrorCallback(errorCallback, "upsertBatch");
			vos = vos.slice();
			var noIdVos:cuboro.tables.SessionVO[] = [];
			...
			errorCallback?:ErrorCallback):void
		{
			...
			{
				kr3m.util.Rand.getSecureString(32, null, (secString) =>
				{
					id = secString;
		...
		*/
		public getByIds(
			ids:string[],
			callback:(vosById:{[id:string]:cuboro.tables.SessionVO}) => void,
			errorCallback?:ErrorCallback):void
		{
			...
			var sql = "SELECT * FROM `sessions` WHERE `id` IN (?)";
			...
			errorCallback?:ErrorCallback):void
		{
			...
			var sql = "SELECT * FROM `sessions` WHERE `id` = ? LIMIT 0,1";
			...
			db.fetchRow(sql, (row) =>
			{
				...
				row.__proto__ = kr3m.util.Factory.getInstance().map(cuboro.tables.SessionVO).prototype;
				row.postLoad();
				callback(row);
			}, errorCallback);
		}
		...
		public getByLastUpdated(
			lastUpdated:number,
			callback:CB<cuboro.tables.SessionVO[]>,
			errorCallback?:ErrorCallback):void
		{
			errorCallback = this.wrapErrorCallback(errorCallback, "finderName");
			var sql = "SELECT * FROM `sessions` WHERE `lastUpdated` = ?";
			sql = db.escape(sql, [lastUpdated]);
			db.fetchAll(sql, (rows:any[]) =>
			{
				for (var i = 0; i < rows.length; ++i)
				{
...
var tSessions = new cuboro.tables.SessionsTable();
